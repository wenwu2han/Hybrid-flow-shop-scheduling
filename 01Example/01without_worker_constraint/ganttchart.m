...
load resultt0205×2I5_all.mat
load AAA_t0205×2.mat
solution=1;
chrom_decode_one=individul1.decode;
...
for i=1:total_ope_num
    ...
    ST_ope=chrom_decode_one{i,1}(1,5);                                    %取出该工序开始加工时间
    CT_ope=chrom_decode_one{i,1}(1,6);                                    %取出该工序结束加工时间
    if i>job_num
...
makespan=individul1(solution).objectives(1);
chrom_ma=individul1(solution).chromesome(total_ope_num+1:2*total_ope_num);
