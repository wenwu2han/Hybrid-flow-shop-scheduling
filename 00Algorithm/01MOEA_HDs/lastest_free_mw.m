function [chrom_ma_stage,chrom_wa_stage,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode] = lastest_free_mw(stage_rank,job_num,chrom_os_stage1,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode,mach_set_stage,worker_set_stage,Basic_infor)
...
for jj=1:job_num
    ...
    [~,col1] = find(idle_time_m(2,:)<=CT_pre);
    if size(col1,2)>=2
    ...
    [~,col2] = find(idle_time_w(2,:)<=CT_pre);
    if size(col2,2)>=2
