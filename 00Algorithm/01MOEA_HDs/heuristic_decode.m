...
for i=1:pop_size
    ...
    if chromosome(job_num+1)==0
        ...
        for j=1:stage_num
            ...
            if strcmp(Disp1,'LB')||strcmp(Disp1,'LIB')
                [chrom_ma_stage,chrom_wa_stage,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode]=load_balancing(j,job_num,chrom_os_stage1,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode,mach_set_stage,worker_set_stage,Basic_infor,Disp1);
            elseif strcmp(Disp1,'FIMW')
                [chrom_ma_stage,chrom_wa_stage,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode]=first_idle_mw(j,job_num,chrom_os_stage1,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode,mach_set_stage,worker_set_stage,Basic_infor);
            else
                [chrom_ma_stage,chrom_wa_stage,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode] = lastest_free_mw(j,job_num,chrom_os_stage1,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode,mach_set_stage,worker_set_stage,Basic_infor);
            end
            if strcmp(Disp2,'0')
                [chrom_os_stage1, pro_time_array] = FCFS(pro_time_array, j, job_num);
            end
            if j ~= stage_num
                chrom_os(j*job_num+1:(j+1)*job_num)=chrom_os_stage1;
            end
