function [chrom_os_stage,chrom_ma_stage,chrom_wa_stage,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode] = lastest_free_mw_EDD(stage_rank,job_num,chrom_os_stage1,pro_time_array,load_machine_cell,load_worker_cell,chrom_decode,mach_set_stage,worker_set_stage,Basic_infor)
...
if stage_rank ==1
    for jj=1:job_num
        ...
        [~,col1] = find(idle_time_m(2,:)<=T_ready);
        if size(col1,2)>=2
            [~,col_ma] = max(idle_time_m(2,col1));
            mach_rank = idle_time_m(1,col1(col_ma));
            CT_ma = idle_time_m(2,col1(col_ma));
        else
        ...
        [~,col2] = find(idle_time_w(2,:)<=T_ready);
        if size(col2,2)>=2
            [~,col_wa] = max(idle_time_w(2,col2));
            worker_rank = idle_time_w(1,col2(col_wa));
            CT_wa = idle_time_w(2,col2(col_wa));
        else
    ...
    pro_time_array2 = pro_time_array1;
    for jj=1:job_num
        T_ready = pro_time_array2(2,jj);
        for k = 1:m
            idle_time_m(1,k) = max_mach_rank+k;
            if isempty(load_machine_cell{max_mach_rank+k,2})
                CT_ma1 = 0;
