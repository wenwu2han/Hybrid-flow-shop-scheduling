...
dvar boolean isprow[Jobs][Stgs][rWorks];
...
forall (i in Jobs, j in Stgs: j>1)
  t_c[i][j] - t_c[i][j-1] >= (sum (s in 1..nbWork[j]) (sum (k in 1..nbMach[j]) (ispro[i][j][k][s]*OpDurations[i][nbMach[j-1]+k])));
forall (i in Jobs)
...
forall (i in Jobs, b in Jobs, j in Stgs)
  t_c[b][j] - sum (s in 1..nbWork[j]) (sum (k in 1..nbMach[j]) (ispro[b][j][k][s]*OpDurations[b][nbMach[j-1]+k])) + (1-(sum(k in 1..nbMach[j]) pre_m[i][b][j][k]))*B  >= t_c[i][j];

forall (b in Jobs, j in Stgs)
  t_c[b][j] - sum (s in 1..nbWork[j]) (sum (k in 1..nbMach[j]) (ispro[b][j][k][s]*OpDurations[b][nbMach[j-1]+k])) + (1-(sum(k in 1..nbMach[j]) pre_mf[b][j][k]))*B >= 0;

cons3:// prevent the overlapping of any two jobs by the same worker and define the sequence of the jobs
forall (i in Jobs, b in Jobs, j in Stgs)
  t_c[b][j] - sum (s in 1..nbWork[j]) (sum (k in 1..nbMach[j]) (ispro[b][j][k][s]*OpDurations[b][nbMach[j-1]+k])) + (1-(sum(s in 1..nbWork[j]) pre_w[i][b][j][s]))*B  >= t_c[i][j];

forall (b in Jobs, j in Stgs)
  t_c[b][j] - sum (s in 1..nbWork[j]) (sum (k in 1..nbMach[j]) (ispro[b][j][k][s]*OpDurations[b][nbMach[j-1]+k])) + (1-(sum(s in 1..nbWork[j]) pre_wf[b][j][s]))*B >= 0;
...
forall (b in Jobs, j in Stgs)
  forall (s in 1..nbWork[j])
    (sum(i in Jobs: i != b) pre_w[i][b][j][s]) + pre_wf[b][j][s] == isprow[b][j][s];
...
forall (i in Jobs, j in Stgs)
  forall (s in 1..nbWork[j])
  sum(k in 1..nbMach[j]) ispro[i][j][k][s] == isprow[i][j][s];
